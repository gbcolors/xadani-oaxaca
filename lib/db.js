...
function getPhysicalMenuImageMap() {
  return {
    ...
    "Garnachas": "/assets/xadani-portada-foto-11.jpg",
    "Lisa al horno con aderezo": "/assets/xadani-portada-foto-10.jpg",
...
async function syncPhysicalMenuItems() {
  ...
  if (version.rows[0]?.value === "20260605-menu-fisico-v4") return;
  ...
  await query(
    `insert into app_settings (key, value, updated_at)
     values ('physical_menu_sync_version', '20260605-menu-fisico-v4', now())
     on conflict (key)
     do update set value=excluded.value, updated_at=now()`
  );
}
