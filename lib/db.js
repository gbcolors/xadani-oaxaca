...
async function initializeDatabase() {
  ...
  await query(`
    ...
    create table if not exists team_email_accounts (
      id serial primary key,
      ...
      incoming_host text default '',
      incoming_port integer default 993,
      outgoing_host text default '',
      outgoing_port integer default 465,
      security_type text default 'SSL/TLS',
      webmail_url text default '',
    ...
    create table if not exists mobile_app_projects (
      id serial primary key,
      site_key text not null,
    ...
    alter table team_email_accounts add column if not exists temporary_password text default '';
    alter table team_email_accounts add column if not exists incoming_host text default '';
    alter table team_email_accounts add column if not exists incoming_port integer default 993;
    alter table team_email_accounts add column if not exists outgoing_host text default '';
    alter table team_email_accounts add column if not exists outgoing_port integer default 465;
    alter table team_email_accounts add column if not exists security_type text default 'SSL/TLS';
    alter table team_email_accounts add column if not exists webmail_url text default '';
    alter table team_email_accounts add column if not exists signature text default '';
...
async function seedManagedSites() {
  const sites = [
    [
      "xadani",
      "Xadani en Oaxaca",
      ...
      "restaurant",
      "active",
      "https://www.xadanienoaxaca.com",
      "https://www.xadanienoaxaca.com/admin",
      "https://www.xadanienoaxaca.com/reservas.html",
      "Sitio operativo con reservas, menu, galeria, experiencias y panel admin."
    ],
    [
      "verdemar",
      "Verdemar Restaurante & Bar",
      "verdemar.com.mx",
      "restaurant",
      "active",
      "https://verdemar.com.mx",
      "https://verdemar.com.mx/admin",
      "https://verdemar.com.mx/reservas.html",
      "Sitio Verdemar conectado al superadmin para acceso operativo."
    ]
  ];
...
function getPhysicalMenuItems() {
  return [
    ...
    ["fuertes-camarones", "Camarones al horno", "Seis camarones preparados al horno de barro, a elegir natural, al mojo de ajo, enchipotados o con aderezo de chipotle; servidos con ensalada de verduras.", 250, "", "Horno"],
    ...
    ["fuertes-especiales-istmenos", "Garnachas istmeñas", "Tortillas de nixtamal con carne, cebolla, col en vinagre, chile curtido y salsa de chipotle.", 150, "", "Istmo"],
    ...
    ["fuertes-pesca-del-dia", "Lisa al horno de barro tradicional con aderezo de la casa", "Lisa horneada en barro, preparada al estilo tradicional de la casa; se sirve con ensalada de verduras. Precio según tamaño.", 0, "S/T", "Horno"],
...
function getPhysicalMenuImageMap() {
  return {
    "Aguachile de camarón": "/assets/xadani-portada-foto-18.jpg",
    "Ensalada de mariscos": "/assets/xadani-hero-portada.jpg",
    "Garnachas istmeñas": "/assets/gallery/xadani-menu-confirmado/garnachas-istmenas.jpg",
    "Camarones al horno": "/assets/gallery/xadani-menu-confirmado/camarones-al-horno.jpg",
    "Lisa al horno de barro tradicional con aderezo de la casa": "/assets/gallery/xadani-menu-confirmado/lisa-al-horno-barro.jpg",
    "Salmón del patrón": "/assets/xadani-portada-foto-02.jpg"
  };
}
...
async function syncPhysicalMenuItems() {
  ...
  if (version.rows[0]?.value === "20260703-menu-confirmado-v1") return;

  const items = getPhysicalMenuItems();
  const physicalNames = items.map((item) => String(item[1]).toLowerCase());
      ...
      await query(
        `update menu_items
            set category=$2, description=$3, price=$4, price_label=$5, tags=$6,
                ...
                image=case when $8 <> '' then $8 else image end,
                updated_at=now()
          where id=$1`,
        [existing.rows[0].id, category, description, Number(price || 0), priceLabel || "", tags, index, image]
      );
    } else {
      await query(
        `insert into menu_items (category, name, description, price, price_label, image, tags, active, sort_order)
         values ($1,$2,$3,$4,$5,$6,$7,true,$8)`,
        [category, name, description, Number(price || 0), priceLabel || "", image, tags, index]
      );
    }
  }
  ...
  await query(
    `insert into app_settings (key, value, updated_at)
     values ('physical_menu_sync_version', '20260703-menu-confirmado-v1', now())
     on conflict (key)
     do update set value=excluded.value, updated_at=now()`
  );
}
  ...
  const users = [
    {
      ...
      password: process.env.SUPERADMIN_PASSWORD || "coco2380",
      role: "owner",
      recoveryEmail: process.env.SUPERADMIN_RECOVERY_EMAIL || process.env.ADMIN_RECOVERY_EMAIL || "",
      forcePassword: true
    },
    {
    ...
    await query(
      `insert into admin_users (username, salt, password_hash, role, recovery_email)
       ...
       do update set role = excluded.role,
                     recovery_email = coalesce(nullif(admin_users.recovery_email, ''), excluded.recovery_email),
                     salt = case when $6 then excluded.salt else admin_users.salt end,
                     password_hash = case when $6 then excluded.password_hash else admin_users.password_hash end,
                     updated_at = now()`,
      [user.username, salt, passwordHash, user.role, user.recoveryEmail, Boolean(user.forcePassword)]
    );
  }
}
...
function signAdminToken(username, role = "") {
  ...
  const payload = role ? `${username}.${role}.${issuedAt}` : `${username}.${issuedAt}`;
...
function getAdminRoleFromToken(token) {
  if (!verifyAdminToken(token)) return "";
  const parts = String(token).split(".");
...
async function getAdminUserFromToken(token) {
  ...
  const tokenRole = getAdminRoleFromToken(token);
  if (tokenRole) {
    return {
      username,
      role: tokenRole,
