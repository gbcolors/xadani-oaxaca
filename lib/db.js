...
async function seedGalleryItems() {
  const count = await query("select count(*)::int as total from gallery_items");
  if (count.rows[0]?.total > 0) return;

  const items = [
    ["Como iniciamos", "Cocina familiar istmeña, horno, nixtamal y recetas que se comparten en mesa.", "concepto", "https://images.unsplash.com/photo-1556911220-bff31c812dba?auto=format&fit=crop&w=1200&q=82"],
    ["Nuestra cocina", "Producto fresco, adobos de la casa y preparación al momento.", "cocina", "https://images.unsplash.com/photo-1551218808-94e220e084d2?auto=format&fit=crop&w=1200&q=82"],
    ["Xadani en Oaxaca", "Un restaurante familiar para comer sin prisa y probar sabores del Istmo.", "instalaciones", "https://images.unsplash.com/photo-1555396273-367ea4eb4db5?auto=format&fit=crop&w=1200&q=82"]
  ];

  for (const [index, item] of items.entries()) {
    await query(
      `insert into gallery_items (title, caption, type, image, sort_order)
       values ($1, $2, $3, $4, $5)`,
      [item[0], item[1], item[2], item[3], index]
    );
  }
}
...
async function seedExperiences() {
  ...
  await query(
    `insert into experiences (title, description, image, event_date, event_time, price, payment_type, cta_label, sort_order)
     values ($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
    [
      "Experiencia Xadani permanente",
      "Mesa especial con platos al horno, sabores istmeños y atención familiar. Configurable para reservar o prepagar.",
