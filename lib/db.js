...
function getPhysicalMenuItems() {
  return [
    ...
    ["entradas-frias", "Camarones en aguachile", "Camarones con limón, pepino, cebolla y chile habanero; una entrada fresca y vibrante.", 250, "", "Picante"],
    ...
    ["fuertes-camarones", "Camarón al horno", "Seis camarones preparados al horno, a elegir natural, mojo de ajo, enchipotado o aderezo de chipotle; servidos con ensalada de verduras.", 250, "", "Horno"],
    ["fuertes-camarones", "Camarón al mojo de ajo frito", "Seis camarones al mojo de ajo, acompañados con ensalada de verduras, arroz a la jardinera y aderezo de chipotle.", 250, "", "Ajo"],
    ...
    ["entradas-cocteles", "Cóctel de camarón", "Camarón en salsa de cátsup con cilantro, cebolla y aguacate. Disponible en tamaño mediano o grande.", 200, "M $200 / G $250", "Fresco"],
    ...
    ["fuertes-pesca-del-dia", "Lisa al horno", "Lisa preparada al horno a elegir natural, mojo de ajo, a la mexicana, enchipotlada o con aderezo de chipotle; servida con ensalada de verduras.", 0, "S/T", "Horno"],
    ...
    ["fuertes-pesca-del-dia", "Salmón al orégano al horno", "Salmón horneado con orégano, aceite de oliva y ajo; acompañado con ensalada de verduras y arroz a la jardinera.", 350, "", "Horno"]
  ];
}
...
async function syncPhysicalMenuItems() {
  ...
  if (version.rows[0]?.value === "20260605-menu-fisico-v1") return;
  ...
  await query(
    `insert into app_settings (key, value, updated_at)
     values ('physical_menu_sync_version', '20260605-menu-fisico-v1', now())
     on conflict (key)
     do update set value=excluded.value, updated_at=now()`
  );
}
  ...
  await query(`
    with ordered as (
      select id, row_number() over (order by sort_order, id) as rn
        from gallery_items
       where active = true
         and (title ~* '^IMG ' or title = 'Cocina y mesa Xadani' or coalesce(caption, '') = '')
    )
    update gallery_items g
       set title = case ordered.rn
             when 1 then 'Mesa istmeña al centro'
             when 2 then 'Sabores de Santa María Xadani'
             when 3 then 'Del horno a la mesa'
             when 4 then 'Cocina familiar del Istmo'
             when 5 then 'Mar, maíz y sazón'
             when 6 then 'Especialidades de la casa'
             when 7 then 'Platos para compartir'
             when 8 then 'Texturas del Istmo'
             when 9 then 'Horno, fuego y tradición'
             when 10 then 'Detalle de nuestra cocina'
             when 11 then 'Mesa para familias y grupos'
             when 12 then 'Recetas servidas sin prisa'
             when 13 then 'Producto fresco de la región'
             when 14 then 'La carta visual de Xadani'
             when 15 then 'Momentos en Xadani'
             when 16 then 'Cocina con raíz familiar'
             when 17 then 'Especiales de temporada'
             when 18 then 'El sabor del Istmo'
             else 'Xadani en Oaxaca'
           end,
           caption = case ordered.rn
             when 1 then 'Una mesa generosa pensada para compartir al centro, con sabores del Istmo y espíritu familiar.'
             when 2 then 'Recetas inspiradas en Santa María Xadani, servidas en Oaxaca con sazón de casa.'
             when 3 then 'Preparaciones al horno con carácter istmeño, producto fresco y guarniciones abundantes.'
             when 4 then 'Cocina tradicional de familia: platos directos, cálidos y hechos para encontrarse.'
             when 5 then 'Una propuesta donde conviven pescado, camarón, maíz, hierbas y salsas de la región.'
             when 6 then 'Selección de platillos especiales de la casa, preparados para disfrutarse sin prisa.'
             when 7 then 'Platos amplios para convivencias, celebraciones y mesas familiares.'
             when 8 then 'Crujiente, fresco, horneado y especiado: contrastes que definen nuestra carta.'
             when 9 then 'El horno como centro de la experiencia, con preparaciones de raíz istmeña.'
             when 10 then 'Detalles de cocina que muestran el cuidado de cada plato y cada servicio.'
             when 11 then 'Un espacio para grupos, familias y visitantes que buscan una experiencia auténtica.'
             when 12 then 'Recetas familiares servidas con calma, abundancia y el sabor de la casa.'
             when 13 then 'Ingredientes frescos de la región trabajados con sazón autóctono.'
             when 14 then 'Imágenes de nuestros platillos, instalaciones y momentos alrededor de la mesa.'
             when 15 then 'Instantes de la experiencia Xadani: comida, reunión y atención cercana.'
             when 16 then 'Una cocina con memoria familiar y vínculo con el Istmo de Oaxaca.'
             when 17 then 'Platillos y preparaciones especiales que cambian según producto y temporada.'
             when 18 then 'El sabor del Istmo servido en Oaxaca, con platos para compartir.'
             else 'Fotografía del concepto Xadani en Oaxaca.'
           end,
