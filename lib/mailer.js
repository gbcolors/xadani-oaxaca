...
async function command(socket, text, expectedCodes = [], label = text) {
  socket.write(`${text}\r\n`);
  const response = await readLine(socket);
  if (expectedCodes.length && !expectedCodes.some((code) => response.startsWith(String(code)))) {
    throw new Error(`SMTP ${label} failed: ${sanitizeSmtpResponse(response)}`);
  }
  return response;
}
...
async function sendMail({ to, subject, text }) {
  ...
  try {
    ...
    await command(socket, Buffer.from(config.user).toString("base64"), [334], "AUTH username");
    await command(socket, Buffer.from(config.pass).toString("base64"), [235], "AUTH password");
    ...
    if (!dataResponse.startsWith("250")) {
      throw new Error(`SMTP DATA failed: ${sanitizeSmtpResponse(dataResponse)}`);
    }
